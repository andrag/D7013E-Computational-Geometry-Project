	...
	public void handleEventPoint(Endpoint p){ 
		...
		if(p.isLower){
			...
			for(Edge e : p.getLowerTo()){//Tries to delete all segments the lower point refers to but there is only one segment left in the tree
				...
				//2016-03-31: IMPORTANT! We must also delete e from its uppers isUpperTo array. Not done so far. I think.
				//An attempt at this is performed below
				//Not sure if it is needed though. Maybe getUpperTo is not checked for points that are passed.
				Endpoint theUpperOfEdge = e.getUpper();
				int indexOfSegmentToErase = -1;
				for(Edge segment : theUpperOfEdge.getUpperTo()){
					if(segment.compareTo(e) == 0){//Is this comparing valid? Think so.
						indexOfSegmentToErase = theUpperOfEdge.getUpperTo().indexOf(segment);
					}
				}
				if(indexOfSegmentToErase != -1){
					theUpperOfEdge.getUpperTo().remove(indexOfSegmentToErase);
				}
			}
