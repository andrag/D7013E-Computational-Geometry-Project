...
public class Main {
	...
	/* This test test that the compareTo method places all segments in the correct order.
	 * It considers only endpoints as uppers and lowers. No intersections or handleNewEvents are tested.
	 * 
	 * Important rules when handling the status:
	 * 		1. current_X and sweep_Y must always be updated when using any operation in the status. 
	 * 				- calculateCurrentX() or some method can be made in the Edge class
	 * 
	 * 		2. When re-ordering segments after intersection. Just remove and add one edge and they will switch places automatically. As long as current_X and sweep Y are kept updated.
	 * 
	...
	/*private static void compareToTest()
	{
		...
		Edge edge3 = new Edge(p1, p4, id++);
		edge3.current_X = p1.getX();
		edge3.sweep_Y = sweep_y; //Keep swep_Y at the top since we now check uppers
		
		System.out.println("Element 1 of isLowerTo of p4: " + p4.isLowerTo.get(0));
		System.out.println("Element 2 of isLowerTo of p4: " + p4.isLowerTo.get(1));
		...
		//1. Update the sweep line position and the current_X of each segment
		
		System.out.println("Updating the current_Xs and sweep_Y.");
		
		// This approach is a problem! It only works when manually switching places on the segments that should switch places beforehand. Which i beleive the code does.

		status.remove(edge3);
		sweep_y = 400 - 100; //Should always be compared to realY
		
		edge1.sweep_Y = sweep_y;
		edge2.sweep_Y = sweep_y;
		
		edge3.sweep_Y = sweep_y;
		edge3.current_X = 75;
		...
		//Two edges should switch places here
		status.add(edge3);//Bug!
		
		
		//2. Check that the treeSet has reordered the elements
		status.traverseStatus();
		
		//3. Remove all edges that has a lower in p2 (edge1 and edge4)
		status.remove(edge1);
		...
		status.remove(edge3);
		p2.isLowerTo.remove(edge4);
		p3.isUpperTo.remove(edge4);
		
		status.traverseStatus();
		
		//4. Add a new horizontal edge between p2 and p4
		System.out.println("Adding horizontal edge6");
		Edge edge6 = new Edge(p2, p4, id++);
		edge6.current_X = 50;
		edge6.sweep_Y = sweep_y;
		status.add(edge6);
		...
		System.out.println("==================================TEST 6====================================");
		//Handle lower point - remove edge4 and 2
		status.remove(edge4);
